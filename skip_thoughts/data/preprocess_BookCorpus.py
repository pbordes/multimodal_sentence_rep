...
tf.flags.DEFINE_string("input_files", "/local/bordes/datasets/BookCorpus/books_large_p*.txt",
...
tf.flags.DEFINE_string("output_dir", "/local/bordes/binaries/bookcorpus_data", "Output directory.")
...
tf.flags.DEFINE_boolean("eval", False,
                        "Whether to build evaluation sets.")
...
def _process_input_file(filename, vocab, stats):
  ...
  """
  tf.logging.info("Processing the input file: %s", filename)
  processed = []
...
def main(unused_argv):
  ...
  if FLAGS.eval:
    ...
    tf.logging.info("Build evaluation file for SICK evaluation.")

    max_len = 50
    s1_train, s1_test, s2_train, s2_test, scores_train, scores_test = [], [], [], [], [], []

    loc = "/home/bordes/skip_thoughts/eval_data"
    with open(os.path.join(loc, 'SICK_train.txt'), 'rb') as f:
        for line in f:
            text = line.strip().split('\t')
            s1_train.append(text[1].decode('utf8'))
            s2_train.append(text[2].decode('utf8'))
            scores_train.append(text[3])

    with open(os.path.join(loc, 'SICK_test_annotated.txt'), 'rb') as f:
        for line in f:
            text = line.strip().split('\t')
            s1_test.append(text[1].decode('utf8'))
            s2_test.append(text[2].decode('utf8'))
            scores_test.append(text[3])
            
    scores_train = [float(s) for s in scores_train[1:]]
    scores_test = [float(s) for s in scores_test[1:]]
    s1 = s1_train[1:] + s1_test[1:]
    s2 = s2_train[1:] + s2_test[1:]
    scores = scores_train + scores_test
    
    n_s = len(s1) #nb of sentences
    S1_ids = np.zeros([n_s,max_len])
    S2_ids = np.zeros([n_s,max_len])
    S1_mask = np.zeros([n_s,max_len])
    S2_mask = np.zeros([n_s,max_len])
    
    for k in range(n_s):
      S1_ids[k,:len(s1[k].split())] =  _sentence_to_ids(s1[k].lower().split(), vocab)[:max_len]
      S2_ids[k,:len(s2[k].split())] =  _sentence_to_ids(s2[k].lower().split(), vocab)[:max_len]
      S1_mask[k,:len(s1[k].split())] =  np.ones([len(s1[k].split())])[:max_len]
      S2_mask[k,:len(s2[k].split())] =  np.ones([len(s2[k].split())])[:max_len]

    np.save("/home/bordes/skip_thoughts/skip_thoughts/evaluation/SICK_s1_ids.npy",S1_ids)
    np.save("/home/bordes/skip_thoughts/skip_thoughts/evaluation/SICK_s2_ids.npy",S2_ids)
    np.save("/home/bordes/skip_thoughts/skip_thoughts/evaluation/SICK_s1_mask.npy",S1_mask)
    np.save("/home/bordes/skip_thoughts/skip_thoughts/evaluation/SICK_s2_mask.npy",S2_mask)
    np.save("/home/bordes/skip_thoughts/skip_thoughts/evaluation/SICK_scores.npy", np.array(scores) )

    tf.logging.info("Build evaluation file for STS evaluation.")

    s1_total,  s2_total,  scores_total = [], [], []

    for setting in ["deft-forum", "deft-news", "headlines", "images", "OnWN", "tweet-news"] :

      max_len = 50
      s1,  s2,  scores = [], [], []

      with open(os.path.join(loc, 'STS-'+setting+'.txt'), 'rb') as f:
          for line in f:
              text = line.strip().split('\t')
              s1.append(text[0].decode('utf8'))
              s2.append(text[1].decode('utf8'))
              scores.append(float(text[2]))
      ...
      np.save("/home/bordes/skip_thoughts/skip_thoughts/evaluation/STS-"+setting+"_s1_ids.npy",S1_ids)
      np.save("/home/bordes/skip_thoughts/skip_thoughts/evaluation/STS-"+setting+"_s2_ids.npy",S2_ids)
      np.save("/home/bordes/skip_thoughts/skip_thoughts/evaluation/STS-"+setting+"_s1_mask.npy",S1_mask)
      np.save("/home/bordes/skip_thoughts/skip_thoughts/evaluation/STS-"+setting+"_s2_mask.npy",S2_mask)
      np.save("/home/bordes/skip_thoughts/skip_thoughts/evaluation/STS-"+setting+"_scores.npy", np.array(scores) )
    
    setting = "all"
    s1, s2, scores = s1_total, s2_total, scores_total
    n_s = len(s1) #nb of sentences
