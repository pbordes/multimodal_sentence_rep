...
output_dir = "/local/bordes/binaries/O_data_" + FLAGS.textual
...
def _create_serialized_example(current, vocab, obj):
    """Helper for creating a serialized Example proto."""
    example = tf.train.Example(features=tf.train.Features(feature={
      "encode": _int64_feature(_sentence_to_ids(current, vocab)),
      "feature": _float_feature(obj)
  }))
...
def _process_input_file(filename, vocab, stats):
  ...
  """
    ...
    for successor_str in tf.gfile.FastGFile(filename):
        ...
        try:
            #obj_id = [int(successor_str.split("\t")[1][:-1])]
            #l = successor_str.split("\t")[1][1:-2]
            #obj_id = map(lambda x: float(x), l.split(","))
            obj_id = int(successor_str.split("\t")[1][:-1])

            successor = successor_str.split("\t")[0]
            successor = successor.translate(None, string.punctuation)
            successor = successor.split()
            successor = [w.lower() for w in successor]
          ...
          # sentences with strictly less than max_sentence_length to pass.
            if FLAGS.max_sentence_length and (len(successor) >= FLAGS.max_sentence_length):
                ...
                serialized = _create_serialized_example(successor, vocab, [obj_id])
...
def main(unused_argv):
    ...
    input_f = ["/home/bordes/eval_model/high_level_obj.txt"]
