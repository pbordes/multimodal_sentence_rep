    ...
    with tf.variable_scope(scope or "gru_cell"):
      with tf.variable_scope("gates"):
        ...
        w_x = tf.get_variable(
            ...
            trainable = self.trainable)
        z_and_r = (_layer_norm(tf.matmul(state, w_h), scope="layer_norm/w_h",trainable=False) +
                   _layer_norm(tf.matmul(inputs, w_x), scope="layer_norm/w_x",trainable=False))
        z, r = tf.split(tf.sigmoid(z_and_r), 2, 1)
      with tf.variable_scope("candidate"):
        w = tf.get_variable(
            "w", [input_dim, num_units], initializer=self._w_initializer,trainable = self.trainable)
        u = tf.get_variable(
            "u", [num_units, num_units], initializer=self._u_initializer,trainable = self.trainable)
        h_hat = (r * _layer_norm(tf.matmul(state, u), scope="layer_norm/u",trainable=False) +
                 _layer_norm(tf.matmul(inputs, w), scope="layer_norm/w",trainable=False))
