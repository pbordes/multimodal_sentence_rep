...
def _load_skip_thoughts_embeddings(checkpoint_path, textual):
  ...
  if textual == "SK":
    word_embedding = reader.get_tensor("w_embedding/word_embedding")
  else:
    word_embedding = reader.get_tensor("word_embedding")
  
  #projection = reader.get_tensor("projection")
  #word_embedding = np.dot(word_embedding, projection)
...
def _expand_vocabulary(skip_thoughts_emb, skip_thoughts_vocab, word2vec,textual):
  ...
  tf.logging.info("Creating embeddings for expanded vocabulary")
...
def build_vocabulary(skip_thoughts_model, skip_thoughts_vocab, word2vec_model, output_dir, textual):

  if not tf.gfile.IsDirectory(output_dir):
    tf.gfile.MakeDirs(output_dir)

  # Load the skip-thoughts embeddings and vocabulary.
  skip_thoughts_emb = _load_skip_thoughts_embeddings(skip_thoughts_model, textual)
  skip_thoughts_vocab = _load_vocabulary(skip_thoughts_vocab)

  # Load the Word2Vec model.
  wordvec = word2vec.KeyedVectors.load_word2vec_format(word2vec_model, binary=True)
